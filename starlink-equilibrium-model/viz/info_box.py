...
KEY DESIGN NOTES (hard-won — don't simplify these away)
========================================================
- LineCollection / ContourSet paths must be collected from coll.get_paths() — NOT just
  ax.get_lines(). The old 9-anchor version missed this and placed boxes on contour curves.
- Text avoidance must check ALL Text objects inside the axes area (clabels, annotations,
  legend text), not just the legend widget. Exclude structural text (title, axis labels,
  tick labels) which live outside the data region.
- Text bboxes are inflated by TEXT_PAD_PX so the box keeps visible clearance even when it
  doesn't technically overlap a label.
- QuadMesh (heatmap background) covers the entire axes but carries no localized information —
  ignore it in the line/scatter pass, or every position scores equally badly.
- Watermarks and figure-level annotations belong at fig.text(...) so they never compete with
  the in-axes placement scan. Never put a watermark inside the axes if you also use mode="on".
- mode="off" shrinks the axes, distorting x/y alignment in multi-panel figures. Prefer
  mode="on" for panels; only fall back to mode="off" for standalone charts where one axis
  is genuinely too crowded.
"""
...
def _line_and_scatter_px(ax, fig):
    ...
    the latter is what the old 9-anchor version missed, letting boxes land on contour curves.
    QuadMesh (heatmap background) is ignored — it covers everything so it gives no useful signal.
    """
    ...
    for coll in ax.collections:
        if isinstance(coll, QuadMesh):
            continue                       # heatmap background covers everything — ignore it
...
def _text_bboxes(ax, fig):
    ...
    Bboxes are inflated by TEXT_PAD_PX on each side for visible clearance."""
    renderer = fig.canvas.get_renderer()
...
def _add_off_plot(ax, fig, text, *, fontsize, side, frac):
    ...
    The anchor sits a small FIXED pad from the new axes edge (not scaled by frac) so
    the box hugs the edge with a modest gap, instead of floating centered in the
    margin with dead space on both sides -- the old frac-scaled offset put the
    anchor near the margin's midpoint, wasting roughly half the reserved space.
    """
    fig.canvas.draw()
    box = ax.get_position()
    pad = 0.035
