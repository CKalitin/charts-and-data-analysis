...
SEGMENT_COLORS = {"incumbent": "#762a83", "new": "#2ca25f"}
SEGMENT_LABELS = {"incumbent": "Captured from incumbents (already-connected switchers)",
                   "new": "Newly-connected (previously unconnected)"}
...
# --------------------------------------------------------------------------------------
def fig_revenue_heatmap_by_country(rows: list[ctmf.CountryTAMFull], country_paths):
...
def fig_tam_full_vs_satellites_stacked_by_segment(sat_counts, by_segment: dict[str, np.ndarray]):
    fig, ax = render.new_figure(figsize=(12, 7.5))
    segments = ["new", "incumbent"]  # newly-connected at bottom -- the smaller, more volatile series
    ...
    total = by_segment["incumbent"] + by_segment["new"]
    peak_n = sat_counts[np.argmax(total)]
    incumbent_share_at_peak = by_segment["incumbent"][np.argmax(total)] / total[np.argmax(total)]
    info_box.add_info_box(
        ax, fig,
        f"Peak total TAM {_usd_formatter(total.max(), None)}/mo near N={peak_n:,.0f} --\n"
...
def export_tam_full_csv(telecom_rows: list[dict], household_size: dict[str, float],
    ...
    continent_path)."""
    rows_per_bucket = {
        n: ctmf.compute_country_tam_full(n, telecom_rows, household_size, country_pop_by_lat, global_lat_centers,
...
def main():
    ...
    telecom_rows = ctmf.load_telecom_rows()
    household_size = ctmf.load_household_size()
    ...
    revenue_rows = ctmf.compute_country_tam_full(REVENUE_MAP_SATS, telecom_rows, household_size, pop_by_lat,
                                                  lat_centers, hist, dens_centers)
    country_paths = load_country_paths()
    fig, path = fig_revenue_heatmap_by_country(revenue_rows, country_paths)
    render.save_fig(fig, path)
    print(f"  wrote {path.relative_to(Path(__file__).resolve().parent.parent)}")

    sat_counts = np.geomspace(100, 2_000_000, 30)
    tam = ctmf.sweep_total_tam_full(sat_counts, telecom_rows, household_size, pop_by_lat, lat_centers, hist,
                                     dens_centers)

    fig, path = fig_tam_full_vs_satellites(sat_counts, tam)
    render.save_fig(fig, path)
    print(f"  wrote {path.relative_to(Path(__file__).resolve().parent.parent)}")

    sat_counts_linear = np.linspace(1, TAM_LINEAR_MAX_SATS, 60)
    tam_linear = ctmf.sweep_total_tam_full(sat_counts_linear, telecom_rows, household_size, pop_by_lat, lat_centers,
                                            hist, dens_centers)
    fig, path = fig_tam_full_vs_satellites_linear(sat_counts_linear, tam_linear)
    render.save_fig(fig, path)
    print(f"  wrote {path.relative_to(Path(__file__).resolve().parent.parent)}")

    by_region = ctmf.sweep_tam_full_by_region(sat_counts, telecom_rows, household_size, pop_by_lat, lat_centers,
                                               hist, dens_centers)
    fig, path = fig_tam_full_vs_satellites_stacked_by_region(sat_counts, by_region)
    render.save_fig(fig, path)
    print(f"  wrote {path.relative_to(Path(__file__).resolve().parent.parent)}")

    by_segment = ctmf.sweep_tam_full_by_segment(sat_counts, telecom_rows, household_size, pop_by_lat, lat_centers,
                                                 hist, dens_centers)
    fig, path = fig_tam_full_vs_satellites_stacked_by_segment(sat_counts, by_segment)
    render.save_fig(fig, path)
    print(f"  wrote {path.relative_to(Path(__file__).resolve().parent.parent)}")

    print("wrote 5 charts")

    country_path, continent_path = export_tam_full_csv(telecom_rows, household_size, pop_by_lat, lat_centers, hist,
                                                         dens_centers)
