...
- **`charts/raan_dv.py`** — `outputs/raan/raan_dv_<dep_epoch>.png`: both
  families' minimum-achievable ΔV plotted against ΔRAAN on one shared axis
  (clamped to ±90°, where both curves' minima and the bulk of their rise
  toward the ΔRAAN=±90° worst case are visible), each family's global
...
- **`charts/orbit_geometry.py`** — `outputs/geometry/`: Earth-centered 2D/3D
  ...
  definitions above visually concrete, the same role the deleted
  `departure_geometry_3d.py` played for the old ψ model):
  - `01_earth_both_families.png` — both families' own optimal planes shown
    together in one 3D scene (translucent patches + parking-orbit circles),
    with `v_Earth` and `v∞` arrows, to show how close (but not identical)
    the two families' chosen planes are to each other for this baseline.
  - `02_departure_3d_equatorial.png` / `03_departure_3d_ecliptic.png` — 3D
    view of one family's own best plane: parking orbit, burn point, ΔV
    arrow, and the post-burn hyperbola, plus `v_Earth`/`v∞` with their
    out-of-plane angles labeled.
  - `04_departure_topdown_equatorial.png` / `05_departure_topdown_ecliptic.png`
    — true orthographic top-down (face-on to that plane) view of the same
    burn. Unlike the old ψ model, this plane is only guaranteed to contain
    `v_Earth` exactly at ΔRAAN=0 — at each family's actual optimum (a few
